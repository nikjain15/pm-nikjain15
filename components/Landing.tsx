...
/**
 * The demo video — the product's "how it works" moment. A muted 16:9 loop autoplays
 * (poster shown until it does), with one tap to the full version with sound.
 *
 * Media lives in public/media and is generated by the Remotion project in
 * pulse-video/ (outside this app, so it never touches the build or gate).
 */
function SeeItWork() {
  ...
  return (
    <section className="mt-12 border-t border-zinc-800 pt-8">
      <h2 className="text-xl font-medium tracking-tight text-zinc-100">
        Watch Pulse move a card the moment you ship.
      </h2>
      <p className="mt-3 text-sm text-zinc-400">
        A merged pull request, an answer from the agent, your own private workflow, one shared
        brain across your tools — in about a minute. Nobody types a status update.
      </p>

      <figure className="relative mt-6 overflow-hidden rounded-lg border border-zinc-800 bg-black shadow-2xl">
        <div className="relative aspect-video">
          {full ? (
            <video
              ...
              autoPlay
              playsInline
              poster="/media/pulse-hero.jpg"
              aria-label="Pulse — full demo with sound."
            >
              <source src="/media/pulse-demo.mp4" type="video/mp4" />
            </video>
          ) : (
            <>
              <video
                ...
                preload="metadata"
                poster="/media/pulse-hero.jpg"
                aria-label="Pulse demo — a merged pull request moves a card to Done on its own."
              >
                <source src="/media/pulse-hero.mp4" type="video/mp4" />
              </video>
              <button
                type="button"
                onClick={() => setFull(true)}
                className="absolute bottom-4 left-1/2 inline-flex -translate-x-1/2 items-center gap-2 rounded-full border border-white/15 bg-zinc-950/60 px-5 py-2.5 text-sm font-medium text-zinc-100 backdrop-blur transition-colors hover:border-emerald-400 hover:bg-zinc-950/80"
              >
                <span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_0_4px_rgba(52,211,153,0.18)]" aria-hidden />
                Watch the full version with sound
              </button>
            </>
          )}
        </div>
